...
const loginUser = async (req, res) => {
    try {
        ...
        user ? user.password === crypto.SHA256(req.body.password).toString() ? res.json({ message: "Login Success", email: req.body.email, status: 200 }) : res.json({ message: "Password Incorrect", status: 400 }) : res.json({ message: "User doesn't Exist", status: 400 })
    }
    catch (e) {
        console.log(e)
